- 模糊控制
  ...
  - 模糊推理：
    ...
    - 合成运算方法：Zadeh-最大最小，Kaufmann-最大-乘积下面的符合运算为 max-min，$A\rightarrow B解释为A与B相关$
...
- 神经网络：一种旨在模仿人脑结构及其功能的信息处理系统
  ...
  - 前馈神经网络
    ...
    - BP算法
      ...
      - 改进
        - 动量项 $\Delta w(t)=\eta\delta x+\alpha\Delta w(t-1)$
        - 自适应调节学习率 $\eta=\beta\eta$
        - 陡度因子 $o=\frac{1}{1+e^{-net/\lambda}}$
      - 设计
        - 输入量
          - 两个基本原则
          - 分类：数值变量，语言变量
  ...
  - 自组织神经网络
    ...
    - 步骤：向量归一化；寻找获胜神经元（欧式距离最小，点积最大）；网络输出与权值调整（只有获胜神经元可以调整）；循环直到学习率衰减到0
    - SOM网
      - 拓扑结构：两层
      - 获胜神经元---优胜邻域：开始很大，不断收缩，最后为0
      - Kohonen学习算法
        - 初始化；接受输入；寻找获胜节点；定义优胜邻域；调整权值；结束检查（$\eta$）
